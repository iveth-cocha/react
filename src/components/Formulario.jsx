...
export const Formulario = ({setEstado,idMetro}) => {
    ...
    const handleChange =(e)=>{
        setform({
            ...form, //copiar propiedades del objeto inicial
            [e.target.name]: e.target.value.trim() //actualizar las propiedades;targetname viene del atributo name y lo asocia con su valor
        })
    }

    const handleSubmit = async(e)=>{
        e.preventDefault()
        //validar si el form tiene campos vacios, longitud
        if (Object.values(form).includes("") || Object.entries(form).length === 0)
        {
            setError(true)//cambia el valor de la variable error
            setTimeout(() => {
                setError(false) //cambiar el valor de la variable despues de 1segundo
            }, 1000);
            return
        }
        try {
            ...
            await fetch(url,{
                method:'POST', //accion
                body:JSON.stringify(form), //formato json para la infor
